...
public class PgDbInput implements MyDbInput {
	...
	public PgDbInput(String connectionString, int fetchSize, String query, String user, String password) throws SQLException {
        ...
        try{
        	...
        	System.out.println("cann't connect to DB");
        	close();
        	throw e;
        }
	}
	...
	public String readHeaders() {
        ...
        for (int i = 1; i <= columnCount; i++)
			try {
				rowHeader.append(metadata.getColumnName(i) + SEPERATOR);
			} catch (SQLException e) {
				e.printStackTrace();
			}
	...
	public String next(){
		try {
		        ...
		        for (int i = 1; i <= columnCount; i++)
		            row.append(rs.getString(i) + SEPERATOR);
		        return row.substring(0,row.length()-1) + '\n';
			
		} catch (SQLException e) {
        	System.out.println("cann't get next row");
        	e.printStackTrace();
        	return null;
		}
	}
	...
	public void close() {
    	try {
    		...
    		System.out.println("cann't close DB");
		}
	}
	...
	public boolean hasNext() {
		try {
        	...
        	System.out.println("cann't get next");
        	return false;
		}
	}
}
